...
// ── Constants ────────────────────────────────────────────────────────────
// SVG fills/strokes read HSL triples from app/globals.css tokens. Tailwind
// classes don't apply inside SVG attributes; we wrap raw hsl(var(--token)).

const STROKE_COLOR = 'hsl(var(--border))'
const GRID_COLOR = 'hsl(var(--muted))'
const USER_COLOR = 'hsl(var(--primary))'
const USER_FILL = 'hsl(var(--primary) / 0.08)'
const COMP_COLOR = 'hsl(var(--muted-foreground))'
const AXIS_LABEL_COLOR = 'hsl(var(--muted-foreground))'
const VERTEX_INACTIVE = 'hsl(var(--muted-foreground))'
const FONT_FAMILY =
  ...
  return (
    <div ref={wrapRef} className="w-full max-w-full">
            ...
            <g key={`axis-group-${i}`} className="print:opacity-100">
              ...
              {axis.value > 0 && (
                <text
                  ...
                  y={vy - 6}
                  textAnchor="middle"
                  fill={axisActive ? USER_COLOR : VERTEX_INACTIVE}
                  fontSize={7}
                  fontFamily={FONT_FAMILY}
      ...
      <div className="flex items-center gap-4 text-[10px] print:text-[9px]">
        ...
        {comparison && (
          <span className="flex items-center gap-1.5 text-muted-foreground-dim">
            <span
        ...
        <span className="flex items-center gap-1 text-muted-foreground-dim">
