...
- Name: home.spec.ts >> Landing Page — Full E2E >> Open Source Section >> renders Passion Projects with entries
- Location: e2e/home.spec.ts:184:9
...
Locator: getByText('kidpen.org')
Expected: visible
Error: strict mode violation: getByText('kidpen.org') resolved to 2 elements:
    1) <span class="text-[15px] font-bold text-primary">kidpen.org</span> aka getByRole('button', { name: 'kidpen.org — Free STEM' })
    2) <a target="_blank" href="https://kidpen.org" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 text-[10px] font-mono text-primary no-underline hover:text-primary/80 transition-colors">…</a> aka getByRole('link', { name: 'kidpen.org' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('kidpen.org')
...
> 189 |       await expect(page.getByText(first.name)).toBeVisible()
  ...
  245 |                !msg.includes('installhook') &&
  246 |                !msg.includes('metamask') &&
  247 |                !msg.includes('fullscreen') &&
  248 |                !msg.includes('maxlistenerse exceeded') &&
  249 |                !msg.includes('orphaned data') &&
  250 |                !msg.includes('malformed chunk') &&
  251 |                !msg.includes('the resource at') &&
  252 |                !msg.includes('deprecated')
  253 |       })
  254 |       expect(appErrors).toEqual([])
  255 |     })
  256 |   })
  257 | })
  258 | 
