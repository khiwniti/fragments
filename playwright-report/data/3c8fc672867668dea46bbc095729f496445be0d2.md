...
- Name: home.spec.ts >> Landing Page — Full E2E >> Skills Section >> switching category tab shows different skills
- Location: e2e/home.spec.ts:139:9

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('Full-Stack') resolved to 5 elements:
    1) <p class="text-sm text-muted-foreground max-w-md mx-auto">Hi, I'm Ikkyu — an AI-Augmented Full-Stack Develo…</p> aka getByText('Hi, I\'m Ikkyu — an AI-')
    2) <p class="text-[10px] text-muted-foreground/60 text-center mt-2">Ask about Khiw (Ikkyu) Nitithadachot — AI-Augment…</p> aka getByText('Ask about Khiw (Ikkyu)')
    3) <div class="text-[11px] text-muted-foreground leading-[1.5] mb-1.5">Full-stack building management with 3D viewer</div> aka getByRole('button', { name: 'View details for Facility' })
    4) <button class="px-3 py-1.5 rounded-md text-[11px] font-medium transition-all bg-muted/50 text-muted-foreground hover:text-foreground hover:bg-muted">Full-Stack</button> aka getByRole('button', { name: 'Full-Stack' })
    5) <p class="text-[11px] text-muted-foreground font-mono">Built by Khiw (Ikkyu) Nitithadachot · AI-Augmente…</p> aka getByText('Built by Khiw (Ikkyu)')

Call log:
  - waiting for getByText('Full-Stack')
...
> 144 |       await page.getByText(secondCat.category).click()
  ...
  199 |       await expect(page.getByText(/Have a project in mind/)).toBeVisible()
  200 |       // Check social link labels
  201 |       for (const link of SOCIAL_LINKS) {
  202 |         await expect(page.getByText(link.title)).toBeVisible()
  203 |       }
  204 |     })
  205 | 
  206 |     test('shows Start a Chat button with link to /chat', async ({ page }) => {
  207 |       await setupPage(page)
  208 |       await scrollToSection(page, 'Get in Touch')
  209 |       const chatBtn = page.getByText('Start a Chat')
  210 |       await expect(chatBtn).toBeVisible()
  211 |       await expect(chatBtn).toHaveAttribute('href', '/chat')
  212 |     })
  213 |   })
  214 | 
  215 |   // ── ScrollIndicator ───────────────────────────────────────────────────
  216 |   test.describe('ScrollIndicator', () => {
  217 |     test('shows scroll indicator at top of page', async ({ page }) => {
  218 |       await setupPage(page)
  219 |       // Before scrolling, the indicator should be visible
  220 |       await page.waitForTimeout(500)
  221 |       await expect(page.getByText('Scroll')).toBeVisible()
  222 |     })
  223 |   })
  224 | 
  225 |   // ── Footer ────────────────────────────────────────────────────────────
  226 |   test.describe('Footer', () => {
  227 |     test('renders footer with credit text', async ({ page }) => {
  228 |       await setupPage(page)
  229 |       await scrollToSection(page, 'AI-Augmented Full-Stack Developer')
  230 |       await expect(page.getByText(/Built by Khiw/)).toBeVisible()
  231 |       await expect(page.getByText(/Next.js/)).toBeVisible()
  232 |     })
  233 |   })
  234 | 
  235 |   // ── Console Errors ────────────────────────────────────────────────────
  236 |   test.describe('Console errors', () => {
  237 |     test('no JavaScript errors during page load', async ({ page }) => {
  238 |       await setupPage(page)
  239 |       // Allow some time for async operations to complete
  240 |       await page.waitForTimeout(2000)
  241 |       // Filter out benign MetaMask extension noise
  242 |       const appErrors = consoleErrors.filter((e) => {
  243 |         const msg = e.message.toLowerCase()
  244 |         return !msg.includes('moz-extension') &&
```